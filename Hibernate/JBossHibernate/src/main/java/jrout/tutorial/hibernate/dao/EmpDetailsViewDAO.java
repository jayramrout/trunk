package jrout.tutorial.hibernate.dao;

// Generated Mar 15, 2015 5:19:15 PM by Hibernate Tools 4.0.0

import java.util.List;

import jrout.tutorial.hibernate.model.EmpDetailsView;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.hibernate.LockMode;
import org.hibernate.criterion.Example;

/**
 * Home object for domain model class EmpDetailsView.
 * @see jrout.tutorial.hibernate.model.EmpDetailsView
 * @author Hibernate Tools
 */
public class EmpDetailsViewDAO extends BaseDAO {

	private static final Log log = LogFactory.getLog(EmpDetailsViewDAO.class);

	public void persist(EmpDetailsView transientInstance) {
		log.debug("persisting EmpDetailsView instance");
		try {
			getSession().persist(transientInstance);
			log.debug("persist successful");
		} catch (RuntimeException re) {
			log.error("persist failed", re);
			throw re;
		}
	}

	public void attachDirty(EmpDetailsView instance) {
		log.debug("attaching dirty EmpDetailsView instance");
		try {
			getSession().saveOrUpdate(instance);
			log.debug("attach successful");
		} catch (RuntimeException re) {
			log.error("attach failed", re);
			throw re;
		}
	}

	public void attachClean(EmpDetailsView instance) {
		log.debug("attaching clean EmpDetailsView instance");
		try {
			getSession().lock(instance, LockMode.NONE);
			log.debug("attach successful");
		} catch (RuntimeException re) {
			log.error("attach failed", re);
			throw re;
		}
	}

	public void delete(EmpDetailsView persistentInstance) {
		log.debug("deleting EmpDetailsView instance");
		try {
			getSession().delete(persistentInstance);
			log.debug("delete successful");
		} catch (RuntimeException re) {
			log.error("delete failed", re);
			throw re;
		}
	}

	public EmpDetailsView merge(EmpDetailsView detachedInstance) {
		log.debug("merging EmpDetailsView instance");
		try {
			EmpDetailsView result = (EmpDetailsView) sessionFactory
					.getCurrentSession().merge(detachedInstance);
			log.debug("merge successful");
			return result;
		} catch (RuntimeException re) {
			log.error("merge failed", re);
			throw re;
		}
	}

	public EmpDetailsView findById(jrout.tutorial.hibernate.model.EmpDetailsViewId id) {
		log.debug("getting EmpDetailsView instance with id: " + id);
		try {
			EmpDetailsView instance = (EmpDetailsView) sessionFactory
					.getCurrentSession().get(
							"jrout.tutorial.hibernate.model.EmpDetailsView", id);
			if (instance == null) {
				log.debug("get successful, no instance found");
			} else {
				log.debug("get successful, instance found");
			}
			return instance;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
		}
	}

	public List findByExample(EmpDetailsView instance) {
		log.debug("finding EmpDetailsView instance by example");
		try {
			List results = getSession()
					.createCriteria("jrout.tutorial.hibernate.model.EmpDetailsView")
					.add(Example.create(instance)).list();
			log.debug("find by example successful, result size: "
					+ results.size());
			return results;
		} catch (RuntimeException re) {
			log.error("find by example failed", re);
			throw re;
		}
	}
}
